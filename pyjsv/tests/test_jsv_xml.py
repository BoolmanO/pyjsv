...
class TestSimpleXml(unittest.TestCase):
    def test_convert(self):
        sx = SimpleXml.from_file(path.join(path_to_samples, "test_xml_1.xml"))
        sx2 = SimpleXml.from_dict(sx.get_dict())
        self.assertEqual(sx2.get_xml(), sx.get_xml())
        self.assertEqual(sx2.get_dict(), sx.get_dict())
        
    def test_convert_from_dict(self):
        sx_writer = SimpleXml.from_file(path.join(path_to_samples, "test_xml_1.xml"))
        path_to_tempfile = path.join(path_to_temp, unique_filename() + ".xml") 
        sx_writer.save_file(path_to_tempfile)
        
        sx_reader = SimpleXml.from_file(path_to_tempfile)
        sx_dict = SimpleXml.from_dict(sx_reader.get_dict())
